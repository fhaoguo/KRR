...
'''
Filename : triples.py
Function : Preprocess source TransX and transform to standard fotmat
==> Generally, there are several steps as following:
==> Step1: Transform raw TransX to standard format, some supported format lists as following:
==>        1.CSV, TXT, TAR, ZIP: Use pandas to read(pd.read_csv())
==>        2.JSON: Use json to load and transform to standard format
==>        When dealing with the raw TransX, several exceptions should be considered:
...
==> Step2: Use TransX of standard format to generate entity and relation dict
==> Step3: Split train, evaluation and test TransX if necessary.
'''
...
def csvToStandard(rawPath, savePath="./TransX/standard.txt", names=None, header=None, sep="\t", encoding="utf-8", compression="infer"):
    print("INFO : Loading TransX of type %s" % os.path.splitext(rawPath)[-1])
    ...
    print("INFO : Save standard TransX to file path : %s" % savePath)
    rawDf.to_csv(savePath, sep="\t", header=None, index=None, encoding="utf-8")
    print("INFO : Successfully saving!")

def jsonToStandard(jsonPaths, savePath="./TransX/standard.txt"):
...
def generateDict(dataPath, dictSaveDir):
    if type(dataPath) == str:
        print("INFO : Loading standard TransX!")
        ...
        print("INFO : Loading a list of standard TransX!")
...
'''
This class is suitable for big TransX and this class just process csv TransX
...
class BigTripleCorpus():
    ...
    def loadBigData(self, chunksize, names, sep=',', header=None, encoding="utf-8", compression="infer"):
        '''
        Load TransX of big-size
        :return: None
        '''
        print("INFO : Loading big raw TransX.")
